...
export default defineBackground(() => {
  ...
  browser.tabs.onUpdated.addListener(async (tabId, changeInfo, tab) => {
    // Trigger scan as soon as the URL is available and changes, no need to wait for 'complete'
  ...
  async function fetchSubdomains(domain: string, apiKey: string) {
    ...
    try {
      ...
      // Return a failure object that the client-side can handle.
  ...
  async function triggerScan(url: string) {
    ...
    if (isAutoScanEnabled && rootDomain) {
      ...
      if (!alreadyScanned && !isIgnored) {
        ...
        // Optionally, send a message to the popup if it's open to add to queue immediately
        browser.runtime.sendMessage({ type: 'ADD_TO_QUEUE', domain: rootDomain }).catch(e => {
            // This will fail if the popup isn't open, which is fine.
        });
